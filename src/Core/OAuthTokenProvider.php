...
class OAuthTokenProvider
{
    ...
    /**
     * @var TokenClient $authClient
     */
    private TokenClient $authClient;
     ...
     * @param TokenClient $authClient The client used to retrieve the OAuth token.
     */
    ...
    {
        $request = new CreateServerSideTokenRequest([
            'clientId' => $this->clientId,
            'clientSecret' => $this->clientSecret,
        ]);

        $tokenResponse = $this->authClient->createServerSideToken($request);

        if ($tokenResponse === null) {
            throw new PayabliException(message: "Expected a token response, but received an empty response.");
        }
